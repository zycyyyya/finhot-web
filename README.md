...
- **对比口径**：涨跌对比始终标注具体基准日期（如「较9月14日 4.97% 上升」），优先取数据源自带的上一期观测——LPR 接口的历史期次、财政部 CSV 的前一交易日、ECB 汇率区间序列；无历史接口的指标则沿用上一次运行落盘的观测
- **基准窗口**：对比基准超过 30 天即判定失效，直接显示「对比基准已超30天，暂不对比」而不是继续引用陈旧数值；LPR 为月度指标，窗口放宽至 35 天以覆盖其 30-31 天的天然间隔
